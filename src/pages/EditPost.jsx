...
const EditPost = () => {
  ...
  const getdata = async () => {
    try {
      ...
      setCategories(res.data.getpost.category);
    } catch (error) {
      console.error(error);
    }
  };
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        toast.info(`An error occurred during the update`, {
          position: toast.POSITION.TOP_RIGHT,
        });
      }
    } catch (error) {
  ...
  if (!title || !post) {
    // Data is still being fetched, render loading state or nothing
    return (
      <div className="min-h-[500px] text-center text-[20px]">Loading...</div>
    );
  }
